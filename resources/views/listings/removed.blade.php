@section('title', title('Zamítnuté inzeráty'))
<x-app-layout>
    <x-slot name="header">
        <h1 class="font-semibold text-xl text-gray-800 leading-tight">
            Zamítnuté inzeráty ({{ $listings->count() }})
        </h1>
    </x-slot>
    ...
    <x-container class="mb-8">
        <x-inner-container>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-3 gap-8">
                @each('listings.partials.listing', $listings, 'listing', 'listings.partials.no-listings-to-remove')
            </div>
        </x-inner-container>
    </x-container>
</x-app-layout>
